...
<div class="row">
	<div class="col-md-6">
		<div class="well">
		    <legend><img src="{$module_dir}logo.gif" alt="" /> {l s='PayPal transaction details' mod='paypalmxcustom'}</legend>

			<table cellpadding="0" cellspacing="0" class="table">
				<tr>
					<td>{l s='Method' mod='paypalmxcustom'}</td>
					<td>{$paypal_mx_transaction_details.source|escape:'htmlall':'UTF-8'}</td>
				</tr>
				<tr>
					<td>{l s='Transaction ID' mod='paypalmxcustom'}</td>
					<td>{$paypal_mx_transaction_details.id_transaction|escape:'htmlall':'UTF-8'}</td>
				</tr>
				<tr>
					<td>{l s='Amount charged' mod='paypalmxcustom'}</td>
				...
				<tr>
					<td>{l s='Mode' mod='paypalmxcustom'}</td>
					<td>{if $paypal_mx_transaction_details.mode == 'test'}<span class="warn">{l s='Sandbox (Test)' mod='paypalmxcustom'}</span>{else}{l s='Live' mod='paypalmxcustom'}{/if}</td>
				</tr>
				<tr>
					<td>{l s='Date' mod='paypalmxcustom'}</td>
				...
				<tr>
					<td>{l s='Credit card type' mod='paypalmxcustom'}</td>
				...
				<tr>
					<td>{l s='Credit expiration date' mod='paypalmxcustom'}</td>
				...
				<tr>
					<td>{l s='Credit card last 4 digits' mod='paypalmxcustom'}</td>
				...
				<tr>
					<td>{l s='CVC Check' mod='paypalmxcustom'}</td>
				...
				<tr>
					<td>{l s='PayPal fees' mod='paypalmxcustom'}</td>
	...
	<div class="col-md-6">
		<div class="well">
			<legend>
				...
				{l s='Proceed to a full or partial refund via PayPal' mod='paypalmxcustom'}
			</legend>
			{if isset($paypal_mx_refund) && $paypal_mx_refund}
				<div class="alert alert-success">{l s='Refund successfully performed' mod='paypalmxcustom'}</div>
			{else}
				{if isset($paypal_mx_refund) && !$paypal_mx_refund}
					<div class="alert alert-danger">{l s='An error occured during this refund' mod='paypalmxcustom'}{if isset($paypal_mx_refund_error) && $paypal_mx_refund_error} - {$paypal_mx_refund_error|escape:'htmlall':'UTF-8'}{/if}</div>
				{/if}
			{/if}
			{if $paypal_mx_more60d}
				<div class="alert alert-info">
					{l s='This order has been placed more than 60 days ago or no transaction details are available. Therefore, it cannot be refunded anymore.' mod='paypalmxcustom'}
				</div>
			{/if}
			<table class="table" cellpadding="0" cellspacing="0">
				<tr>
					<th>{l s='Date' mod='paypalmxcustom'}</th>
					<th>{l s='Amount refunded' mod='paypalmxcustom'}</th>
				</tr>
				...
				{/foreach}
				<tr>
					<td>{l s='Total refunded:' mod='paypalmxcustom'}</td>
			...
			{if $paypal_mx_transaction_details.amount == $total_refund && $total_refund}
				{l s='This order has been fully refunded.' mod='paypalmxcustom'}
			{else}
				<form method="post" action="" name="refund" class="form-inline">
					...
					<div class="form-group">
						<label class="control">
							{l s='Refund:' mod='paypalmxcustom'} 
						</label>
						<input type="text" class="" name="refund_amount" value="{($paypal_mx_transaction_details.amount-$total_refund)|floatval}" />
					</div>
					<div class="form-group">
						<input type="submit" name="process_refund" value ="{l s='Process Refund' mod='paypalmxcustom'}" class="btn btn-primary" />
					</div>
				</form>
			{/if}
		</div>
